...
router.get("/analytics/spend-by-category", asyncHandler(async (req, res) => {
...
router.get("/analytics/category-trend", asyncHandler(async (req, res) => {
  const month = String(req.query.month || "");
  if (!month || !/^\d{4}-\d{2}$/.test(month)) {
    res.status(400).json({ error: "Invalid month format. Expected YYYY-MM." });
    return;
...
router.get("/analytics/cc-dues", asyncHandler(async (req, res) => {
