...
router.post("/surplus/distribute", asyncHandler(async (req, res) => {
...
router.get("/surplus/allocations", asyncHandler(async (req, res) => {
...
router.get("/surplus/can-undo", asyncHandler(async (req, res) => {
  const month = req.query.month as string;
  if (!month || !/^\d{4}-\d{2}$/.test(month)) {
    res.status(400).json({ error: "Invalid month format. Use YYYY-MM." });
    return;
...
router.post("/surplus/undo", asyncHandler(async (req, res) => {
